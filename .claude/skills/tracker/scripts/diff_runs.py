...
def to_telegram(result: dict) -> str:
    """Короткая сводка для уведомления: по строке на изменение.

    Отличается от to_text не данными, а плотностью: в телефон уходит не разбор
    прогона, а ответ на вопрос «что поменялось», который читается с экрана
    блокировки. Поэтому строка самодостаточна — в ней и товар, и что с ним
    произошло, — а ссылки и разбивка по пунктам остаются текстовому формату.
    """
    ...
    for item in result["changes"]:
        # Название товара — подпись строки; нет названия (старый прогон) — ссылка.
        label = item.get("title") or item["url"]
        for line in item["lines"]:
            # Строчная буква после тире: «Насос — цена: …» вместо «Насос — Цена: …».
            out.append(f"• {label} — {line[:1].lower()}{line[1:]}")
    out += ["", summary_line(result)]
    return "\n".join(out)
